...
function delete_subject($connect,$subject_id){
...
function get_subject_for_add($connect, $subject_name){
	$query = sprintf("SELECT * FROM subjects WHERE subject_name='%s' limit 1", $subject_name);
	$result = mysqli_query($connect, $query);
	if(!$result) return false;
	$subject = mysqli_fetch_assoc($result);
	if (isset($subject)) {
